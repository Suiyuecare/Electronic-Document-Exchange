-- Restore the runtime schema that existed in the main Supabase project but was absent from source control.
-- Definitions below were derived from the live catalog on 2026-08-27.
-- This migration is additive/idempotent and intentionally does not touch formal exchange providers.
...
-- Avoid the redundant unique index that duplicated official_editor_document_revision_unique.
drop index if exists public.idx_official_editor_revisions_document_revision;
...
grant select, insert on table public.official_document_dispatch_events to service_role;
grant select on table public.official_document_archive_exports to service_role;
grant select, insert, update on table public.official_workflow_delegations to service_role;
