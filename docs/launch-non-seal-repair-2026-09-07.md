...
## 2026-09-08 接續結果（優先於下列歷史狀態）

[Email 啟用與修復結果](email-activation-result-2026-09-08.md) 已完成：寄件設定已更新，
兩筆原驗收信皆有 provider receipt 且顯示 Delivered；追加式派送結果修復已發布。
完整 513 項測試（2 skip）與 CI `34140994621` 通過；正式部署為
`dpl_73VR9VjuD7xEy6b6QGec4cW7DBpy`。
15 分鐘監控已啟用，相同私密傳輸路徑實測 HTTP 201，未增加通知或重寄 Email。
下列「寄信設定待完成、尚未消耗重試、排程 inactive、需登入 Resend」為先前歷史，已被本節取代。
真人收信／角色驗收、異地取回還原、固定備份排程與金鑰交接仍待完成；不代表全部正式上線門檻通過。
...
本次無本人完成 Google 登入的正式 session，未執行該完整流程，沒有新增登入例外。
即使取得維運簽章秘密，自簽測試 handoff 也不能作為本人 Google 登入或人工簽核的驗收證據。
正式站健康檢查和匿名存取拒絕另行驗證。
...
最新版 receipt：`DRILL-20260907-222256-960a7dd7`。

- 95 張 eDoc 資料表、1,175 筆資料、116 個 RLS 政策。
- 還原至新建、無 TCP 的本機 PostgreSQL，筆數、完整資料 hash、ACL、RLS 相符。
- 73 秒完成；向上取整 RTO 2 分鐘、快照年齡 2 分鐘；來源快照為台北時間 2026-09-07 22:22:57。
- 備份採 AES-256-GCM，另以備份完成離線還原驗證。
- 加密檔 829,476 bytes，整檔 SHA-256：`327a05f87e1eee02a0bf06d56a42a6357c5344db2f9093e9e7592daeebd8f6b2`。
- Receipt 1,419 bytes，整檔 SHA-256：`29fb1aecd9d1719b0c9ce611ad69dcf7a610d153676797d30c362fa5655cf3af`。
- Receipt 內部 canonical hash：`33f153bf9a0542e94147375209094651d0849ab6c0e022e36559e2735aa6fb15`，計算時排除 `receipt_sha256` 欄位，不能當作整檔 hash。
- 兩個私密 bucket 皆無正式物件；不能宣稱已完成正式 PDF／章檔抽樣。

永久加密備份與 receipt 由操作人私密保存，金鑰另置受限目錄，均不在 repo。
依使用者授權自行決定異地位置，已在公司負責人帳號的 My Drive 建立「eDoc 私密異地備份」，
上傳上述加密檔與 receipt。以 Google Drive 技能指定的連接器完成檔案及權限讀回，
確認 parent、檔案大小、MIME type 正確，資料夾及兩檔皆 `shared=false`，只有公司 owner 權限。
未公開分享、未新增付費服務，金鑰未上傳至同一位置。

連接器再次抓取加密檔成功並回報 829,476 bytes，但只提供虛擬檔案參照；
目前工具未提供將該參照物化到本機的能力，metadata 介面亦未回傳 bytes checksum。
因此「私密異地副本已存在」通過，「雲端下載 bytes hash 與該副本的隔離還原」仍未驗證。
不得拿原本的本機檔案代替雲端副本，亦未以公開分享或瀏覽器憑證繞過下載限制。
這不是 Supabase 雲端平台 DR；外部還原 runner、固定備份排程及異機金鑰保管交接仍待完成。
一次快照的時效不能宣稱持續達成 15 分鐘 RPO。

## 仍需真人完成

本輪自主查核細節見 [Email 啟用查核](email-activation-check-2026-09-07.md) 與
[正式角色驗收](real-role-acceptance-2026-09-07.md)。既有免費寄信安裝仍存在，
但官方 SSO 要求帳號擁有者登入，未自行購買新方案或繞過權限。
...
4. 完成異地取回還原、固定備份排程與金鑰交接；確認上線日主責、備援人與聯絡方式。
...
仍待有效寄信設定、真人角色流程驗收、異地取回還原及備份排程／金鑰交接；印章依使用者要求排除。
